...
a Python wrapper for a fast solver generated by FORCES PRO v3.0.1
   ...
   OUTPUT = peterPan_py.peterPan_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = peterPan_py.peterPan_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = peterPan_py.peterPan_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'peterPan/lib/peterPan.so')) 
try:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'peterPan/lib/peterPan_withModel.so'))
	csolver = getattr(_lib,'peterPan_solve')
except:
	_lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'peterPan/lib/libpeterPan_withModel.so'))
	csolver = getattr(_lib,'peterPan_solve')

class peterPan_params_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
	_fields_ = [('xinit', ctypes.c_double * 10),
('x0', ctypes.c_double * 50),
('all_parameters', ctypes.c_double * 50),
]

peterPan_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
peterPan_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
}

class peterPan_outputs_ctypes(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
class peterPan_info(ctypes.Structure):
#	@classmethod
#	def from_param(self):
#		return self
...
csolver.argtypes = ( ctypes.POINTER(peterPan_params_ctypes), ctypes.POINTER(peterPan_outputs_ctypes), ctypes.POINTER(peterPan_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def peterPan_solve(params_arg):
	'''
a Python wrapper for a fast solver generated by FORCES PRO v3.0.1
 ...
 See also COPYING
	...
	# convert parameters
	params_py = peterPan_params_ctypes()
	for par in params_arg:
		try:
			#setattr(params_py, par, npct.as_ctypes(np.reshape(params_arg[par],np.size(params_arg[par]),order='A'))) 
			params_arg[par] = np.require(params_arg[par], dtype=peterPan_params_types[par], requirements='F')
	...
	outputs_py = peterPan_outputs_ctypes()
	info_py = peterPan_info()
	if sys.version_info.major == 2:
		...
		try:
			PyFile_AsFile.restype = ctypes.POINTER(FILE)
			exitflag = _lib.peterPan_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.peterPan_casadi2forces )
		...
		try:
			if sys.platform.startswith('win'):
				exitflag = _lib.peterPan_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.peterPan_casadi2forces)
			else:
				exitflag = _lib.peterPan_solve( params_py, ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.peterPan_casadi2forces)
			libc.fclose(fp)
	...
	# convert outputs
	for out in peterPan_outputs:
		peterPan_outputs[out] = npct.as_array(getattr(outputs_py,out))

	return peterPan_outputs,int(exitflag),info_py

solve = peterPan_solve
