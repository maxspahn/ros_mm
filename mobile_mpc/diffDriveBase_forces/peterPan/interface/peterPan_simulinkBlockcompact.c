...
#define S_FUNCTION_NAME peterPan_simulinkBlockcompact
...
typedef peterPaninterface_float peterPannmpc_float;

extern void peterPan_casadi2forces(double *x, double *y, double *l, double *p, double *f, double *nabla_f, double *c, double *nabla_c, double *h, double *nabla_h, double *hess, solver_int32_default stage, solver_int32_default iteration);
peterPan_extfunc pt2function_peterPan = &peterPan_casadi2forces;
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	/* Input Port 1 */
    ssSetInputPortMatrixDimensions(S,  1, 50, 1);
    ssSetInputPortDataType(S, 1, SS_DOUBLE);
    ssSetInputPortComplexSignal(S, 1, COMPLEX_NO); /* no complex signals suppported */
    ssSetInputPortDirectFeedThrough(S, 1, 1); /* Feedthrough enabled */
    ssSetInputPortRequiredContiguous(S, 1, 1); /*direct input signal access*/	
	/* Input Port 2 */
    ssSetInputPortMatrixDimensions(S,  2, 50, 1);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	/* Solver data */
	peterPan_params params;
	peterPan_output output;
	peterPan_info info;	
	...
	/* Call solver */
	exitflag = peterPan_solve(&params, &output, &info, fp , pt2function_peterPan);
	...
	for( i=0; i<10; i++)
	{ 
		outputs[k++] = (real_T) output.x2[i]; 
	}

	k=20; 
	for( i=0; i<10; i++)
	{ 
		outputs[k++] = (real_T) output.x3[i]; 
	}

	k=30; 
	for( i=0; i<10; i++)
	{ 
		outputs[k++] = (real_T) output.x4[i]; 
	}

	k=40; 
	for( i=0; i<10; i++)
	{ 
		outputs[k++] = (real_T) output.x5[i]; 
	}
