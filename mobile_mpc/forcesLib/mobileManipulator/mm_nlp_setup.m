...
nbSelfCollision = 1;
nbPlanes = 1;
model.nh = (nbObstacles + nbPlanes) * nbSpheres + nbSelfCollision;   % number of inequality constraint functions
...
elseif strcmp(dynamics, 'simple')
    ...
    n_other_param = 3 + 3 + 7 + 6 + 4 * nbObstacles + 7 * nbPlanes;    % [dt, r, L, x_des, y_des, theta_des, q_des (size : 7), weights, obstacles(1).x, obstacles(1).y, obstacle(3), obsctacles(1).r, ...]
...
elseif strcmp(dynamics, 'acc')
    lower_bound = [-inf, -inf, -pi, -wheel_lim_vel, -wheel_lim_vel, q_lim_franka_low, -q_lim_franka_vel, slack_lim_low, -wheel_lim_acc, -wheel_lim_acc, -q_lim_franka_acc];
    upper_bound = [inf, inf, pi, wheel_lim_vel, wheel_lim_vel, q_lim_franka_up, q_lim_franka_vel, slack_lim_up, wheel_lim_acc, wheel_lim_acc, q_lim_franka_acc];
elseif strcmp(dynamics, 'simple')
...
for i=1:model.N
    ...
    model.hu{i} = inf((nbObstacles + nbPlanes) * nbSpheres + nbSelfCollision, 1);
    model.hl{i} = zeros((nbObstacles + nbPlanes) * nbSpheres + nbSelfCollision, 1);
end
if strcmp(dynamics, 'torques')
